...
def chunk_semantic(text: str, threshold: float = SEMANTIC_THRESHOLD,
    ...
    for i in range(1, len(sentences)):
        emb_prev = embeddings[i - 1]
        emb_curr = embeddings[i]
        norm_prev = norm(emb_prev)
        norm_curr = norm(emb_curr)
        if norm_prev == 0 or norm_curr == 0:
            sim = 0.0
        else:
            sim = dot(emb_prev, emb_curr) / (norm_prev * norm_curr + 1e-9)
        ...
        for sent in p_sentences:
            if len(current_child_text) + len(sent) > child_size and current_child_text:
                children.append(Chunk(
                    ...
                    metadata={**metadata, "chunk_type": "child", "child_index": child_idx},
                    parent_id=pid
                ))
                current_child_text = ""
                child_idx += 1
    ...
    for part in parts:
        ...
        if re.match(r'^#{1,3}\s+', part):
            if current_content.strip() or current_header.strip():
                ...
                chunk_meta = {**metadata, "strategy": "structure"}
                if current_header:
                    chunk_meta["section"] = current_header.strip().lstrip("#").strip()
